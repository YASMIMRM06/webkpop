...
asset('images/default-user.png') }}" alt="{{ auth()->user()->name }}" class="w-full h-full rounded-full object-cover border-4 
border-kpop-pink"> 
                        <form id="profile-picture-form" action="{{ route('users.profile.update') }}" method="POST" 
enctype="multipart/form-data" class="hidden"> 
                            @csrf 
                            <input type="file" name="profile_picture" id="profile-picture-input" accept="image/*"> 
                        </form> 
                        <button onclick="document.getElementById('profile-picture-input').click()" class="absolute bottom-0 right-0 
bg-kpop-blue text-white rounded-full p-2 hover:bg-kpop-purple transition duration-300"> 
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" 
xmlns="http://www.w3.org/2000/svg"> 
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 
001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 
2H5a2 2 0 01-2-2V9z"></path> 
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 13a3 3 0 11-6 0 3 3 0 016 
0z"></path> 
                            </svg> 
                        </button> 
                    </div> 
                    ...
                    <form action="{{ route('users.profile.update') }}" method="POST"> 
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4"> 
                            <div> 
                                ...
                                <input type="text" id="name" name="name" value="{{ old('name', auth()->user()->name) }}" class="input-field 
@error('name') border-red-500 @enderror"> 
                            ...
                            <div> 
                                ...
                                <input type="email" id="email" name="email" value="{{ old('email', auth()->user()->email) }}" class="input-field 
...
auth()->user()->birth_date->format('Y-m-d') : '') }}" class="input-field @error('birth_date') border-red-500 @enderror"> 
                            @error('birth_date') 
                                <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p> 
                            @enderror 
                        </div> 
                        <div class="mb-4"> 
                            ...
                            <textarea id="bio" name="bio" rows="3" class="input-field @error('bio') border-red-500 @enderror">{{ old('bio', 
auth()->user()->extendedProfile->bio ?? '') }}</textarea> 
                            @error('bio') 
                                <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p> 
                            @enderror 
                        </div> 
                        <div class="mb-6"> 
                            <label for="social_networks" class="block text-gray-700 font-bold mb-2">Redes Sociais (separadas por 
vírgula)</label> 
                            <input type="text" id="social_networks" name="social_networks" value="{{ old('social_networks', 
auth()->user()->extendedProfile->social_networks ?? '') }}" class="input-field @error('social_networks') border-red-500 
@enderror"> 
                    ...
                    <ul class="space-y-3"> 
                        @foreach(auth()->user()->participatingEvents as $event) 
                            <li class="border-b border-gray-200 pb-3"> 
                                <a href="{{ route('events.show', $event) }}" class="font-semibold text-kpop-blue hover:underline">{{ 
$event->name }}</a> 
                        ...
                        @foreach(auth()->user()->ratings as $rating) 
                            <li class="border-b border-gray-200 pb-3"> 
                                <div class="flex justify-between"> 
                                    <a href="{{ route('groups.musics.show', [$rating->music->group, $rating->music]) }}" class="font-semibold 
text-kpop-purple hover:underline">{{ $rating->music->title }}</a> 
