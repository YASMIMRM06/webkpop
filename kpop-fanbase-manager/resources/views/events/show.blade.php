                ...
                <div class="flex space-x-3"> 
                    @auth 
                        @if(auth()->user()->isAdmin() || auth()->user()->id === $event->user_id) 
                            ...
                            <form action="{{ route('events.destroy', $event) }}" method="POST" onsubmit="return confirm('Tem certeza que 
deseja excluir este evento?');"> 
                                @csrf 
                                @method('DELETE') 
                                <button type="submit" class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded-full 
transition duration-300">Excluir</button> 
                            </form> 
                        @endif 
                    @endauth 
                </div> 
        ...
        <div class="card"> 
            <div class="p-6"> 
                ...
                <div class="flex items-center space-x-4"> 
                    <img src="{{ $event->creator->profile_picture ? asset('storage/'.$event->creator->profile_picture) : 
asset('images/default-user.png') }}" alt="{{ $event->creator->name }}" class="w-16 h-16 rounded-full object-cover"> 
                    <div> 
                    ...
                    <div class="space-y-3"> 
                        @foreach($event->participants as $participant) 
                            <div class="flex items-center space-x-3"> 
                                <img src="{{ $participant->profile_picture ? asset('storage/'.$participant->profile_picture) : 
asset('images/default-user.png') }}" alt="{{ $participant->name }}" class="w-10 h-10 rounded-full object-cover"> 
                                <div> 
